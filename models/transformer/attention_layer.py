...
class Attention(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    # self.q_dense_layer = tf.keras.layers.Dense(
    #     self.hidden_size, use_bias=False, name="q", kernel_initializer="glorot_normal")
    # self.k_dense_layer = tf.keras.layers.Dense(
    #     self.hidden_size, use_bias=False, name="k", kernel_initializer="glorot_normal")
    # self.v_dense_layer = tf.keras.layers.Dense(
    #     self.hidden_size, use_bias=False, name="v", kernel_initializer="glorot_normal")
    # self.output_dense_layer = tf.keras.layers.Dense(
    #     self.hidden_size, use_bias=False, name="output_transform", kernel_initializer="glorot_normal")
  ...
  def call(self, x, y, bias, training, cache=None):
    ...
    if training:
      weights = tf.nn.dropout(weights,
                              noise_shape=[tf.shape(weights)[0], tf.shape(weights)[1], 1, 1],
