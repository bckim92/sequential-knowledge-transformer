...
class SequenceBeamSearch(object):
  ...
  def __init__(self, symbols_to_logits_fn, vocab_size, batch_size,
               beam_size, alpha, max_decode_length, eos_id, use_copy_decoder):
  ...
  def _grow_alive_seq(self, state):
    ...
    if self.use_copy_decoder:
        candidate_log_probs = tf.math.log(logits)
    else:
    ...
    # top k indices (normal beam search)
...
def sequence_beam_search(
    ...
    alpha, max_decode_length, eos_id, use_copy_decoder):
